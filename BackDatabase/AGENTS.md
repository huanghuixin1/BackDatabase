...
- 清理顺序保持为：写新文件前先删 0 字节文件，再按数据库分别以 `LastWriteTimeUtc` 删除最旧文件。`dbmaxfiles` 为每库覆盖值，未配置时沿用任务级 `maxfiles`。
...
- `maxfiles`/`dbmaxfiles` 按数据库分别生效，但不是写入完成后的严格上限：清理发生在本轮写文件之前，因此该库在本轮结束后可能暂时超过上限 1 个文件。
